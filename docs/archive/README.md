# Archive

This directory stores retired site files that are kept for reference only.

Files here are not part of the active runtime path. Do not use them as evidence
for the live site unless a template, include, or generated page links to them
explicitly.

Active styling is loaded from `_includes/metadata-hook.html`, which points to
`assets/css/NormaiNight.css` (night) and `assets/css/ProsperoLight.css` (day).