...
评论头像使用 Twikoo 输出的 WeAvatar/Gravatar 兼容头像地址；访客如果想显示自定义头像，需要在 WeAvatar 使用评论时填写的同一个邮箱设置头像。站点 CSS 会把评论列表头像显示为小圆角矩形，输入框头像仍保持隐藏，避免挤压表单布局。
...
如果想让一篇文章成为隐藏文章，在 front matter 里加入：
...
| 右侧“兴趣之中”最近修改 | 不显示 |
| 标签页与标签归档 | 显示 |
| 首页监测面板统计 | 计入文章数、总字数、最近更新等 |
| 直接访问文章 URL | 可以正常访问 |

隐藏文章页内的“同频系列”面板会正常显示同分类文章；普通文章页的“同频系列”面板不会列出隐藏文章。
...
关于页的个人监控面板下方还有 `待阅读栈` 和 `待观看栈` 两列，每列最多显示 8 个真实条目。数据也在 `_tabs/about.md` 顶部配置区：
...
每两个条目之间用 `||` 分隔。页面只显示编号、标题和右下角添加时间，不显示评分、状态、作者或导演。
...
触摸手势只识别足够明确的纵向滑动，不拦截页面默认滚动；手机手势打开时不会自动聚焦搜索框，避免直接弹出软键盘。
...
- 中部是搜索框。
- 默认不会主动列出所有 `hidden: true` 文章；只有带 `probe_default: true` 的隐藏文章会作为样例信号默认显示，状态文案不会暴露默认来源或隐藏文章总量。
- 输入搜索内容后，下方显示匹配的隐藏文章。
- 搜索框可以按标题、日期、分类、标签、摘要筛选隐藏文章。
- 正在输入搜索框、评论框、普通输入框时，不会触发方向键秘籍。
- 方向键和触摸手势都不拦截默认滚动，所以输入指令时页面仍可能正常滚动。
