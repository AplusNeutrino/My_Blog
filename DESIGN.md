...
- Daily record order is supplied by the run-level A/B/C perspective ranking. Cards show the perspective score and at most two short evidence-based reasons; v2 fallback scores are visibly marked as legacy rather than relabeled as a current perspective.
...
- `03 · FIELD MAP` is a single-select filter for the `04 · PUBLIC RECORDS` list only. Its native buttons wrap without overflow, retain a 44px touch target, and preserve the run-level ranking after filtering.
- Five deliberate craft details are required: a mono date ledger, three-step signal grammar, evidence-count chips, ranked category filters, and a small LKG/data-freshness seal.
